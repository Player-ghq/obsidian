...
- Anki 字段里的冒号前标签要加粗，例如 `<b>字面含义</b>：`、`<b>考研核心考点</b>：`、`<b>文章语境</b>：`；只加粗冒号前标签，不加粗整段解释。
- 不为了制卡而制卡。每张 Anki 卡必须能对应考研英语一能力点：阅读主旨、细节、推理、态度、词义猜测、篇章逻辑、翻译中的熟词僻义/一词多义/长难句、完形搭配与逻辑、新题型段落衔接、作文迁移。只有文章八卦、低频专名、过窄技术细节、无法迁移的表达，不进 Anki，只放笔记或跳过。
- Anki 标签只能使用 `外刊精读` 一个标签；不要追加文章名、词性、主题、卡片类型或其他标签。
- 制卡模板不是死模板。每张卡按表达本身调整详略，但高价值卡通常要包含：原文表达或拓展词组、中文+英文双语词汇拆解、多义项详细解释及例句、完整英文句子例句和中文翻译、可用于考研作文时的作文应用说明。
