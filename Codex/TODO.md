...
- [ ] TopHubData：纯标准库 Python 导出工具已在 `feature/tophubdata-exporter` 隔离分支完成 HTTP、分页、JSON/CSV 与 CLI，17 项测试通过；真实调用 `GET https://api.tophubdata.com/nodes?p=1` 返回业务码 `100300`（余额不足，请充值）。下一步需充值、提供有余额的密钥，或明确批准改用其他免费数据源后再完成真实导出与分支收尾；密钥未写入仓库或 Obsidian。

- [ ] 电子科技大学830备考：核对2027年招生专业目录与830正式考纲；有可靠来源时补2024–2026真题（现文件缺2017，2024只有占位）。13套原创模拟卷及独立评分已于2026-09-19完成，其中新增D—M十套含实际配图（160题、40道计算题已核验）；现已整合A—M全集及试题/答案合订本，入口 `projects/电子科技大学830数字图像处理备考/模拟试卷/全集-A至M/00-总目录.md`；收到首周摸底或模拟卷答卷后，按分步评分、错因与用时调整每日3小时计划，入口 `projects/电子科技大学830数字图像处理备考/README.md`。
